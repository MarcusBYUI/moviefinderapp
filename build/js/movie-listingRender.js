import{getUrlParams as v}from"./utils.js";export function renderMovies(o,l){const d=l.returnMovies(l.moviesSearch(o));d.then(a=>{const r=a,h=r.Search;r.Error=="Too many results."?document.querySelector(".movie-list-container").innerHTML=`
                ...
                <p>Year: ${t.Year}</p>
                `,s.classList.add("movie-short-info"),i.classList.add("movieChild1"),e.classList.add("movieChild2"),s.appendChild(i),s.appendChild(e),document.querySelector(".movie-list-container").appendChild(s)}const m=+`${r.totalResults}`;if(m>10){const t=+v("page"),s=m%10;let i=m/10;s>0&&(i=Math.floor(i+1));let e=document.createElement("div"),n=document.createElement("button"),g=t-1,c=document.createElement("button"),f=t+1,u=o.substring(0,o.indexOf("page=")+"page".length+1);n.innerHTML="Previous",n.addEventListener("click",p=>{p.preventDefault(),location.href=`../movie-listing/?searching=${u}${g}`}),c.innerHTML="Next",c.addEventListener("click",p=>{p.preventDefault(),location.href=`../movie-listing/?searching=${u}${f}`}),e.classList.add("page-selection"),e.innerHTML=`
