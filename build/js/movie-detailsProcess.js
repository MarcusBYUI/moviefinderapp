...
class g {
  constructor() {
    (this.movieId = d("movie")), (this.services = new u());
  }
  pullMovie() {
    return m(this, null, function* () {
      (this.movie = yield this.services.returnMovies(
        this.services.moviesSearch(this.movieId, !0)
      )),
  ...
  renderMovieDetails() {
    (document.title += ` . ${this.movie.Title}`),
      ...
      (document.getElementsByTagName(
        "meta"
      ).description.content = `Movies details page for ${this.movie.Title}`),
      ...
      (document.querySelector(
        ".details-page-genre"
      ).innerHTML = this.movie.Genre),
      (document.querySelector(
        ".movie-details-description"
      ).innerHTML = this.movie.Plot),
      this.movie.Ratings.length === 1
        ? ((document.querySelector(
            ".details-page-imdb"
    ...
    r == this.movieId &&
      ((e.innerHTML = `<i class = "${s}"></i>`), o.textContent++, (t = !0)),
      n.addEventListener("click", () => {
        t
          ? ((t = !1),
            (e.innerHTML = `<i class = "${a}"></i>`),
            o.textContent--,
            localStorage.removeItem(`liked${this.movieId}`, this.movieId))
          : ((t = !0),
            (e.innerHTML = `<i class = "${s}"></i>`),
            o.textContent++,
